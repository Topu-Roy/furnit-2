      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-4 md:flex-row">
        <div className="w-full space-y-8 pb-4 md:w-1/2 md:pb-0 lg:pl-8 2xl:pl-0">
          <div className="mx-auto space-y-2 px-2 text-center md:pr-14 lg:px-0 lg:text-left">
            <Link href={"/"}>
              <h2 className="text-primary">Furnit.</h2>
            </Link>
            <p className="sm:px-8 md:px-4 lg:px-0">
          ...
          <div className="flex w-full items-center justify-center gap-4 md:pb-4 lg:justify-start">
            {socialLinks.map(item => (
              <Link
                key={item.name}
                href={item.url}
                className="hover:bg-primary/10 hover:text-primary h-10 w-10 rounded-full bg-white/10 p-2 text-gray-300"
              >
                {item.icon}
              </Link>
            ))}
        ...
        <div className="flex flex-1 flex-row px-4 py-8">
          {footerData.map(item => (
            <div key={item.headline} className="flex-1 text-center">
              <p className={cn("pb-4 text-white/70")}>{item.headline}</p>
