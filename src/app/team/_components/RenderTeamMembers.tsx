...
export function RenderTeamMembers(props: props) {
  const socialIcons = [
    {
      name: "Facebook",
      icon: <Facebook />,
  ...
  return (
    <div className="mx-auto grid max-w-7xl grid-cols-2 gap-4 px-2 py-6 md:grid-cols-3 md:px-3 md:py-8 lg:py-14 xl:py-16 2xl:px-0">
      {props.teamMembers.map(member => (
        <div key={`${member.id}-team-member`} className="w-full space-y-4 rounded-xl border p-4 shadow">
            ...
            <div className="absolute bottom-[3%] left-0 z-20 flex w-full translate-y-[150%] flex-row items-center justify-around gap-2 ring-0 transition-all duration-300 group-hover:translate-y-0">
              <div className="flex flex-row items-center justify-between gap-3">
                {socialIcons.map(icon => (
                  <TooltipProvider key={`${icon.name}-social-icon`} delayDuration={300}>
                    <Tooltip>
                      <TooltipTrigger>
                        <Link
                          href={"#"}
          ...
          <div className="flex flex-col items-start justify-start gap-2 lg:flex-row lg:items-center">
            <p className="font-bold tracking-wide text-black/85">{member.name}</p>
            <p className="font-medium text-black/70">({member.role})</p>
          </div>
        </div>
      ))}
    </div>
