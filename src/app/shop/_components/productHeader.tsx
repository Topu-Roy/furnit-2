...
export default function ProductHeader({ sheetOpen, setSheetOpen, products }: Props) {
      ...
      <div className="flex w-full flex-row items-center justify-between gap-1">
        <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
          <SheetTrigger asChild>
            <Button variant={"outline"} className="w-24 space-x-2 text-gray-900/70 lg:hidden">
              <span>Filter</span>
              <Filter />
            </Button>
          </SheetTrigger>
        ...
        <div className="flex max-w-md flex-1 flex-row justify-center rounded-full border border-input bg-white">
          <div className="flex w-full flex-row justify-center gap-2 pl-1">
            ...
            />
            <Button onClick={() => setSearchText("")} variant={"outline"} className="h-10 rounded-full p-2.5">
              <X
